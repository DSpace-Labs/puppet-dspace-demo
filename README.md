...
This repo includes the setup files for the 'athena.duraspace.org' server. 

This includes the following:
* [`cloud-init.yaml`](https://github.com/duraspace/puppet-athena/blob/master/cloud-init.yaml) : a [cloud-init](https://help.ubuntu.com/community/CloudInit) Config data file (to be passed via userdata) to initialize server in EC2
* [`Puppetfile`](https://github.com/duraspace/puppet-athena/blob/master/Puppetfile) : a [librarian-puppet](http://librarian-puppet.com/) config file used to install all necessary Puppet modules
* [`manifests/site.pp`](https://github.com/duraspace/puppet-athena/blob/master/manifests/site.pp) : the actual Puppet script to run via 'puppet apply' (requires [hiera](http://docs.puppetlabs.com/hiera/1/index.html) data files which are stored in Amazon S3)
...
* Must have an Amazon IAM role created which provides these permissions:
   ...
   * Temporary READ access to the S3 bucket containing Puppet hieradata  

Once the pre-requisites are in place, all you need to do is:

1. Spin up a new Ubuntu 12.04 LTS
...
Here's what the [`cloud-init.yaml`](https://github.com/duraspace/puppet-athena/blob/master/cloud-init.yaml) does for you:
...
6. Clones this 'puppet-athena' GitHub repo to server (at `/etc/puppet/`)
...
10. Ensures the secondary EBS volume (/dev/xvdb) is formatted as ext3 AND mounted to `/space`
